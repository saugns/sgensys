...
typedef struct ParseConv {
	...
	SAU_MemPool *imp; // for temporary data (borrowed from input SAU_Parse)
...
static OpContext *ParseConv_update_opcontext(ParseConv *restrict o,
	...
	OpContext *oc = NULL;
	if (!pod->prev) {
		oc = SAU_MemPool_alloc(o->imp, sizeof(OpContext));
		if (!oc)
			return NULL;
	} else {
...
static bool ParseConv_add_event(ParseConv *restrict o,
	...
	VoContext *vc;
	if (!pe->vo_prev) {
		vc = SAU_MemPool_alloc(o->imp, sizeof(VoContext));
		if (!vc) goto ERROR;
		e->ev_flags |= SAU_SDEV_NEW_OPGRAPH;
	} else {
